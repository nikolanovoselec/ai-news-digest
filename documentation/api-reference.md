...
| `admin.auth.aud_unset_warning` | `Cf-Access-Jwt-Assertion` header is present but `CF_ACCESS_AUD` is unset — the Worker is checking header presence only, not the `aud` claim. Emitted on every admin request in this configuration so the misconfiguration is visible via `wrangler tail` or Logpush. See [Deployment: Setting CF_ACCESS_AUD](deployment.md#setting-cf_access_aud-strongly-recommended-in-production). |
