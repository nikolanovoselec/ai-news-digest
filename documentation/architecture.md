...
| `migrations/0011_article_embeddings.sql` | `embedding_status` ('pending' / 'embedded' / 'failed') and `embedded_at` columns on `articles`. The admin embed-backfill route picks up rows with NULL or `'failed'` status; the chunk consumer stamps `'embedded'` after a successful Vectorize upsert. | [REQ-PIPE-003](../sdd/generation.md#req-pipe-003-same-story-dedupe-across-the-entire-article-history) |
