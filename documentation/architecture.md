...
| `etld.ts` | Naive eTLD+1 helper (`etldPlusOne`, `sameVendor`): extracts the registrable domain from a host for same-publisher detection in the dedup diagnostic. Covers the corpus's dominant TLDs; does not pull the full Public Suffix List. | [REQ-PIPE-003](../sdd/generation.md#req-pipe-003-same-story-dedupe-across-the-entire-article-history) |
...
| `embeddings.ts` | bge-base-en-v1.5 embedding helpers: input builder (`source_snippet` preferred, falls back to `details_json`; length-capped), cosine similarity, threshold parser, same-vendor penalty parser, batch caller for the AI binding | [REQ-PIPE-003](../sdd/generation.md#req-pipe-003-same-story-dedupe-across-the-entire-article-history) |
...
| `src/queue/scrape-finalize-consumer.ts` | Same-story dedupe pass: per-article `VECTORIZE.queryById` (top-K=5), applies same-vendor cosine penalty (`DEDUP_SAME_VENDOR_PENALTY`) when both articles share an eTLD+1, filters by adjusted score vs threshold + older `published_at`, merges auto-merge-band matches into the oldest qualifying article via `mergeAsAltSource`. Borderline-band matches (`[DEDUP_RERANK_FLOOR, DEDUP_COSINE_THRESHOLD)`) go to an LLM same-event judgment via `dedup-rerank.ts` and merge only on a positive verdict. Per-call rerank cap caps worst-case cost. Atomic `finalize_recorded` gate (migration 0010) prevents double-counting on redelivery. | [REQ-PIPE-003](../sdd/generation.md#req-pipe-003-same-story-dedupe-across-the-entire-article-history), [REQ-PIPE-009](../sdd/generation.md#req-pipe-009-llm-re-rank-pass-for-borderline-same-story-candidates) |
...
Finalize consumer (semantic same-story dedupe)
  ...
  │   ├─ Filter matches: id != self.id, adjusted_score >= DEDUP_COSINE_THRESHOLD (adjusted = score - DEDUP_SAME_VENDOR_PENALTY when same eTLD+1), metadata.published_at < self.published_at
