...
| `src/queue/cleanup.ts` | Queue consumer for the daily retention cron — runs two sequential passes: (1) article retention sweep: deletes unstarred articles older than 7 days with FK-cascade cleanup of alternative sources, tag rows, and read-tracking rows ([REQ-PIPE-005](../sdd/generation.md#req-pipe-005-seven-day-retention-with-starred-exempt-cleanup)); (2) orphan-tag KV sweep: deletes `sources:{tag}` and `discovery_failures:{tag}` KV entries whose tag is not owned by any user ([REQ-PIPE-007](../sdd/generation.md#req-pipe-007-orphan-tag-kv-cache-cleanup)) | [REQ-PIPE-005](../sdd/generation.md#req-pipe-005-seven-day-retention-with-starred-exempt-cleanup), [REQ-PIPE-007](../sdd/generation.md#req-pipe-007-orphan-tag-kv-cache-cleanup) |
