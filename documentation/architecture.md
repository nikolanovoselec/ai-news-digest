...
| `src/lib/prompts.ts` | `PROCESS_CHUNK_SYSTEM`, `DISCOVERY_SYSTEM`, prompt builders, `LLM_PARAMS`. The `details` field contract: exactly 3 plaintext paragraphs separated by the JSON-escaped token `\n` (backslash-n), each 3–4 sentences, totalling 200–250 words. Paragraph roles are fixed: 1. WHAT happened (concrete facts, who/what/when), 2. HOW it works (architecture, mechanism, numbers), 3. IMPACT for the reader (cost, migration, security posture). A `details` value under ~180 words is treated as a failed response. `temperature` is 0.5 — enough creative range to write full paragraphs without collapsing to minimum-entropy single-sentence output. | [REQ-PIPE-002](../sdd/generation.md#req-pipe-002-chunked-llm-processing-with-json-output-contract), [REQ-GEN-005](../sdd/generation.md#req-gen-005-single-call-llm-summarization), [REQ-DISC-001](../sdd/discovery.md#req-disc-001-llm-assisted-per-tag-feed-discovery) |
...
Chunk consumer (per chunk, isolated)
  ...
  → LLM enforces 200–250-word HARD CONTRACT on the details field (≥ 180 words required): exactly 3 paragraphs (WHAT happened / HOW it works / IMPACT for the reader), each 3–4 sentences, \n-separated
