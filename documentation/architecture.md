...
| `src/pages/history.astro` | `/history` — calls the `GET /api/history?offset=0` handler in-process (no subrequest), renders paginated digest rows; "Load more" button appends further pages via client-side fetch. When the `?date=YYYY-MM-DD` query parameter is present (deep-linked from the digest grid "see today" tile), the page renders in single-day-focused mode — only matching-day digests are shown and a `.history__back` "Back to all days" control appears above the list. | [REQ-HIST-001](../sdd/history.md#req-hist-001-paginated-past-digests) |
