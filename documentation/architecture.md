...
| `src/lib/prompts.ts` | `DIGEST_SYSTEM`, `DISCOVERY_SYSTEM`, prompt builders, `LLM_PARAMS`. The `details` field in the LLM output contract is plaintext with paragraphs separated by the JSON-escaped token `\n` (backslash-n); the render side un-escapes it to a real newline. This keeps multi-paragraph prose valid inside a single JSON string without post-processing hacks. | [REQ-GEN-005](../sdd/generation.md#req-gen-005-single-call-llm-summarization), [REQ-DISC-001](../sdd/discovery.md#req-disc-001-llm-assisted-per-tag-feed-discovery) |
