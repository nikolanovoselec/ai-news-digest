...
| `src/queue/scrape-finalize-consumer.ts` | Same-story dedupe pass: per-article Vectorize top-K query, time-window pre-filter (`DEDUP_TIME_WINDOW_SECONDS`), same-vendor cosine penalty, two-tier merge logic - auto-merge band (>= `DEDUP_COSINE_THRESHOLD`) and borderline band (>= `DEDUP_RERANK_FLOOR`) where `dedup-rerank.ts` decides via LLM judgment. Bidirectional merge — winner is the older article in the pair regardless of which side just ingested (AD41); the loser-direction is encoded per-candidate so a late-arriving older article still absorbs its already-stored newer match. Enqueues exactly one `dedup-sweep` continuation message scoped to the last 48h after the gate flips, so cross-tick pairs the per-tick pass cannot see (Vectorize indexing latency on same-second siblings, etc.) merge automatically without operator action. Atomic `finalize_recorded` gate prevents double-counting on redelivery. | [REQ-PIPE-003](../sdd/generation.md#req-pipe-003-same-story-dedupe-across-the-entire-article-history), [REQ-PIPE-009](../sdd/generation.md#req-pipe-009-llm-re-rank-pass-for-borderline-same-story-candidates) |
