...
| `src/lib/tag-railing-flip.ts` | Shared FLIP-pattern helper for the tag railing on `/digest` and `/history`. Exports `flipChipToFront(strip, chip, options)` and `isFlipLocked(strip)`. The helper captures pre-mutation rects (FLIP First), moves the tapped chip to slot 0 via `insertBefore` (Last), inverts positions with `transform` (Invert), forces a synchronous layout read to flush pending styles, then transitions transforms to zero on the next animation frame (Play). Applies two classes on tap: `tag-chip--just-tapped` (scale-bounce keyframe, AC 1) is added at the start and removed before the Play phase so CSS animation does not conflict with the inline FLIP transform; `tag-chip--in-flight` (z-index + box-shadow elevation) is added simultaneously and outlasts the cascade so the chip stays visually above its neighbours throughout the slide. Waits a 1-second hold after the pop so the user's eye lands on the popped chip before the cascade begins. **Visible-fraction cascade duration**: the tapped chip's play time is `clamp(TARGET_VISIBLE_CROSSING_MS / visibleFraction, MIN_CASCADE_MS, MAX_CASCADE_MS)` (constants: `TARGET_VISIBLE_CROSSING_MS = 700 ms`, 800 ms min, 3500 ms max). `visibleFraction` is the intersection of the chip's travel range with the strip's visible viewport divided by total travel distance (floored at `MIN_VISIBLE_FRACTION = 0.15`). This keeps the on-screen portion of every cascade at roughly 700 ms regardless of total travel distance — far chips have most of their journey off-screen so their total duration is extended, but only the visible window matters perceptually. **Per-chip easing**: the tapped chip uses `ease-IN` (`cubic-bezier(0.4, 0, 1, 1)`) when its destination is off-screen-left, so the slow phase of the curve covers the visible portion of travel; it uses `ease-OUT` when it arrives within the visible viewport. Displaced chips always use `ease-OUT` (`cubic-bezier(0.2, 0.8, 0.2, 1)`) and a fixed `MIN_CASCADE_MS` floor so they close the gap promptly while the tapped chip continues its longer journey. Sets `data-tag-flip-locked` on the strip for the duration of the cascade to prevent double-tap desyncs (AC 5). Preserves the strip's horizontal scroll position across the entire cascade — no auto-scroll on tap; suppresses the browser's implicit focus-scroll and scroll-snap re-evaluation around `insertBefore` by temporarily blurring the chip, disabling `scroll-snap-type`, and restoring `scrollLeft` if any side-effect mutation moved it (AC 6). **One-shot scroll-down reveal**: after the cascade settles, if the strip is scrolled right, `armScrollDownReveal` attaches a one-time `scroll` listener on `window`; the next downward page scroll triggers a 600 ms easeOutQuint animation of `strip.scrollLeft → 0` so the newly-selected chip is visible docked at slot 0 as the user begins scrolling the dashboard. The in-flight animation is cancellable on `wheel`/`touchstart`/`pointerdown` of the strip itself. Bails to an instant reorder when `prefers-reduced-motion: reduce` is active (AC 8). Lock is cleared in a `try/finally` block so the strip never gets stuck inert if a DOM operation throws. | [REQ-READ-007](../sdd/reading.md#req-read-007-tag-railing-reorder-animation) |
