...
| `src/pages/api/discovery/retry.ts` | `POST /api/discovery/retry` — accepts JSON body `{"tag":"<tag>"}` (returns `200 {ok:true}`) or form-encoded `tag=<tag>` (returns `303 → /settings?rediscover=ok&tag=<tag>`); clears `sources:{tag}` and `discovery_failures:{tag}` KV, re-queues in `pending_discoveries`; gated by Cloudflare Access | [REQ-DISC-004](../sdd/discovery.md#req-disc-004-manual-re-discover) |
...
| `src/pages/settings.astro` | `/settings` — steady-state settings editor; includes `StatsWidget`, `HashtagChip`, `ModelSelect`, a Force Refresh section with live scrape-progress polling via `GET /api/scrape-status`, and a "Stuck tags" fieldset ([REQ-DISC-004](../sdd/discovery.md#req-disc-004-manual-re-discover)) that surfaces a "Re-discover #{tag}" button for every user tag with an empty `sources:{tag}` KV entry; the fieldset is absent when no tag is stuck | [REQ-SET-001](../sdd/settings.md#req-set-001-unified-first-run-and-edit-flow), [REQ-DISC-004](../sdd/discovery.md#req-disc-004-manual-re-discover) |
...
The `/settings` page surfaces a "Stuck tags" fieldset ([REQ-DISC-004](../sdd/discovery.md#req-disc-004-manual-re-discover)) when any of the session user's tags has an empty `sources:{tag}` KV entry. Each button in that fieldset posts to `POST /api/discovery/retry` — via a native HTML form so the flow works in Samsung Browser and in-app webviews — which clears the stale KV entries and re-queues a discovery pass. The route is gated by Cloudflare Access; non-admin users see the Access login prompt rather than the re-queue succeeding.
