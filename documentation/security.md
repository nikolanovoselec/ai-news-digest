...
1. **Optional Layer 0** (when `CF_ACCESS_AUD` is set): request must carry a Cloudflare Access assertion whose `aud` claim matches. The `exp` claim is validated server-side — expired assertions are rejected even if the perimeter would have caught them first. Signature stays at the Access perimeter ([AD29](decisions/README.md#ad29-cloudflare-access-as-opt-in-additive-perimeter-not-security-boundary), [AD44](decisions/README.md#ad44-cloudflare-access-jwt-exp-validation-signature-still-trusted-from-the-perimeter)).
