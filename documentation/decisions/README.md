...
- **Add LLM rerank above the auto-merge threshold.** Rejected as incoherent. Re-ranking above the merge threshold is paying an LLM cost for a band the threshold already claims to trust; if the score there is unreliable, the threshold is mis-placed and should move down. The fix is to raise auto-merge so the new uncertain stripe falls below it.
- **Add the time-window gate without raising the threshold.** Rejected as insufficient. Several false-merge cluster pairs were within 24h of each other; the time window kills the long-tail multi-day spread but the dense same-day case still needs LLM disambiguation, which only happens when the cosine score sits below auto-merge.
...
- The forward-only nature of this fix means existing false-merge clusters in the corpus stay merged. If an operator wants the 13-source cluster un-merged, the manual path is: list `article_sources` rows for the surviving article id, drop the false-positive rows, re-scrape the dropped source URLs, let the next ingestion path embed them as standalone articles.
