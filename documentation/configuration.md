...
| `ratelimit:{routeClass}:{identity}:{windowIndex}` | Integer string (request count) | TTL = window size in seconds | Application-layer rate-limit counters written and read by `src/lib/rate-limit.ts`. `routeClass` is one of `auth_login` (10/60 s/IP, fail-open), `auth_callback` (20/60 s/IP, fail-open), `auth_refresh_ip` (60/60 s/IP, **fail-closed** — pre-validation tier, caps random-cookie spam), `auth_refresh_user` (10/60 s/user, **fail-closed** — post-validation tier, caps a stolen cookie distributed across many IPs), `auth_logout` (5/60 s/IP, fail-open), `article_star` (60/60 s/user, fail-open), `tags_mutation` (30/60 s/user, fail-open). The two `auth_refresh_*` buckets are shared between `POST /api/auth/refresh` and the inline middleware refresh path so an attacker cannot pivot between endpoints. `identity` is the client IP for IP-keyed routes and the user id for user-keyed routes. `windowIndex` is `Math.floor(Date.now() / 1000 / windowSeconds)`. On KV failure the helper logs `rate.limit.kv_error` with `decision: "fail_open"` or `decision: "fail_closed"` per the rule. Implements [REQ-AUTH-001](../sdd/authentication.md#req-auth-001-sign-in-with-a-federated-identity-provider) AC 9, [REQ-AUTH-002](../sdd/authentication.md#req-auth-002-access-token--refresh-token-instant-revocation) AC 5. |
