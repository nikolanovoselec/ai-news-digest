...
| `ratelimit:{routeClass}:{identity}:{windowIndex}` | Integer string (request count) | TTL = window size in seconds | Application-layer rate-limit counters written and read by `src/lib/rate-limit.ts`. `routeClass` is one of `auth_login` (10/60 s, fail-open), `auth_callback` (20/60 s, fail-open), `auth_refresh` (10/60 s/IP, **fail-closed** — a KV outage returns 429 rather than allowing unbounded refresh calls), `auth_logout` (5/60 s/IP, fail-open), `article_star` (60/60 s, fail-open), `tags_mutation` (30/60 s, fail-open). `identity` is the client IP for unauthenticated and IP-keyed routes and the user id for authenticated mutation routes. `windowIndex` is `Math.floor(Date.now() / 1000 / windowSeconds)`. On KV failure the helper logs `rate.limit.kv_error` with `decision: "fail_open"` or `decision: "fail_closed"` per the rule. Implements [REQ-AUTH-001](../sdd/authentication.md#req-auth-001-sign-in-with-a-federated-identity-provider) AC 9, [REQ-AUTH-002](../sdd/authentication.md#req-auth-002-access-token--refresh-token-instant-revocation) AC 5. |
