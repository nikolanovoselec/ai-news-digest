...
| `ratelimit:{routeClass}:{identity}:{windowIndex}` | Integer string | Window size | Rate-limit counters. Rules: `auth_login` 10/60s IP, `auth_callback` 20/60s IP, `auth_refresh_ip` 60/60s IP (fail-closed), `auth_refresh_user` 10/60s user (fail-closed), `auth_logout` 5/60s IP, `article_star` 60/60s user, `tags_mutation` 30/60s user. Implements [REQ-AUTH-001](../sdd/authentication.md#req-auth-001-sign-in-with-a-federated-identity-provider) AC 9. |
