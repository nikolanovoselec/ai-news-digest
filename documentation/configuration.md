...
| `DEDUP_TIME_WINDOW_SECONDS` | `"259200"` | Hard time-window in seconds for same-story matching. Pairs whose `published_at` differ by more than this are skipped before the cosine check, regardless of score. Default 72h. Tune lower to require tighter same-news-cycle co-occurrence; tune higher (or set to a very large number) to disable the time gate without removing the code path. ([REQ-PIPE-003](../sdd/generation.md#req-pipe-003-same-story-dedupe-across-the-entire-article-history) AC 13, [AD39](decisions/README.md#ad39-raise-dedup-auto-merge-threshold-to-088-and-gate-merges-to-a-72h-news-cycle-window)) |
...
The 0.88 default is the post-2026-05-08 calibration ([AD39](decisions/README.md#ad39-raise-dedup-auto-merge-threshold-to-088-and-gate-merges-to-a-72h-news-cycle-window)). The prior 0.78 (from AD36) was tuned against tightly-bounded news-cycle clusters but missed the dense-theme failure mode: independent articles on a broad topic (e.g. "AI agent governance") routinely cosine-match in the 0.78-0.86 band on topical overlap alone, which produced a 13-source false-merge cluster spanning 9 calendar days. 0.88 raises the auto-merge bar above that band, so the 0.70-0.88 stripe goes to the LLM rerank for a binary same-event judgment. The 72h time window is the second half of the gate: same-event clusters publish in one news cycle, so pairs further apart than that are kept distinct regardless of cosine score. If the embedding model changes or the corpus shifts substantially, re-validate against a fresh sample before relying on these numbers. To tune the rerank band, lower `DEDUP_RERANK_FLOOR` to widen it or raise it to narrow it; setting it equal to `DEDUP_COSINE_THRESHOLD` disables rerank without removing the code path.
