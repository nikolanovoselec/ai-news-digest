...
| `DEDUP_SAME_VENDOR_PENALTY` | `"0.05"` | Cosine penalty subtracted before the threshold gate when both candidates resolve to the same eTLD+1. Lifts the effective same-publisher threshold to 0.90 against the 0.85 default to neutralise publisher-style boilerplate that inflates cosines on LLM-summary embeddings. Set `"0"` to disable. ([REQ-PIPE-003](../sdd/generation.md#req-pipe-003-same-story-dedupe-across-the-entire-article-history) AC 11, [AD33](decisions/README.md#ad33-embed-source-text-and-apply-a-same-vendor-cosine-penalty)) |
| `DEDUP_RERANK_FLOOR` | `"0.72"` | Lower bound of the LLM rerank borderline band. Cosines in `[DEDUP_RERANK_FLOOR, DEDUP_COSINE_THRESHOLD)` go to the language model for a binary same-event judgment; below the floor the pair stays distinct without an LLM call. Lower the floor to widen the LLM-judgment band, raise it to narrow it. Set `"0.85"` (matching the auto-merge threshold) to disable rerank entirely. ([REQ-PIPE-009](../sdd/generation.md#req-pipe-009-llm-re-rank-pass-for-borderline-same-story-candidates)) |

The 0.85 default is validated against the production corpus (2026-05-06): same-event article pairs scored 0.81-0.91 pairwise; different events on the same topic scored 0.77-0.84; unrelated topics scored below 0.73. The threshold sits cleanly between the same-event cluster and the different-event band. The 0.72 rerank floor matches the upper edge of "unrelated topics" in the same sweep. If the embedding model changes or the corpus shifts substantially, re-validate against a fresh sample before relying on these numbers.
