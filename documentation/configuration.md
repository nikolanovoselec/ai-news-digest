...
| `CF_ACCESS_AUD` | Optional Cloudflare Access audience tag. When set, `/api/admin/*` additionally enforces a perimeter check (Layer 0): the request must carry a Cloudflare Access assertion whose `aud` claim matches this value. When unset, Layer 0 is skipped entirely and admin is gated by signed-in session + `ADMIN_EMAIL` alone (AD29). |
