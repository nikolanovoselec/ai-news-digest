...
describe('capChunks — REQ-PIPE-001 (CF-012 + CF-073)', () => {
  ...
  it('REQ-PIPE-001: under the cap → returns input unchanged, no warning logged', () => {
    ...
    const out = capChunks(input, 80, 'run-under');
  ...
  it('REQ-PIPE-001: exactly at the cap → returns input unchanged, no warning logged', () => {
    const input = Array.from({ length: 80 }, (_, i) => `chunk-${i}`);
    const out = capChunks(input, 80, 'run-equal');
    expect(out).toBe(input);
  ...
  it('REQ-PIPE-001: 81 chunks → returns exactly 80 + emits one coordinator_chunks_capped warning', () => {
    const input = Array.from({ length: 81 }, (_, i) => `chunk-${i}`);
    const out = capChunks(input, 80, 'run-over');
    expect(out).toHaveLength(80);
    // The dropped chunk is the last one ("chunk-80").
    expect(out[79]).toBe('chunk-79');
    ...
    expect(payload).toContain('"total_chunks":81');
    expect(payload).toContain('"kept_chunks":80');
  ...
  it('REQ-PIPE-001: heavy overage drops every excess chunk in one slice', () => {
    ...
    const out = capChunks(input, 80, 'run-heavy');
    expect(out).toHaveLength(80);
    expect(out[79]).toBe(79);
