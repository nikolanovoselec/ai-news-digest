...
describe('runHistoricalDedupBatch — REQ-PIPE-003', () => {
  ...
  // ---------------------------------------------------------------
  // REQ-PIPE-023 AD48 — watermark skip on the borderline rerank path.
  ...
  it('REQ-PIPE-023 AD48: both pair members predate watermark — LLM skipped', async () => {
    const aiRun = vi.fn();
    const { result } = await callBatch({
  ...
  it('REQ-PIPE-023 AD48: one pair member newer than watermark — LLM called', async () => {
  ...
  it('REQ-PIPE-023 AD48: bypassWatermark=true always calls LLM regardless of timestamps', async () => {
    const aiRun = vi.fn().mockResolvedValue({
      response: '{"verdicts":[{"i":0,"same_event":true}]}',
    });
    const { result, fixture } = await callBatch({
