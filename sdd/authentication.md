...
**Acceptance Criteria:**
1. Every refresh-token row records a **device fingerprint** at issuance — a hash of the requesting device's User-Agent and country — captured as forensic metadata for future anomaly-detection work. A subsequent refresh on the steady-state path that produces a different fingerprint is logged but is NOT rejected on that signal alone: the cookie's HttpOnly + Secure + `__Host-` prefix, single-use rotation (AC 2), reuse detection (AC 4), and 30-day absolute expiry are the binding mechanisms that protect the session, in line with the OAuth 2.0 BCP guidance. Locking the session to a User-Agent string would lock legitimate users out every time their browser auto-updates, so the steady-state fingerprint mismatch is informational only. The fingerprint check IS retained on the 30-second concurrent-rotation grace branch (AC 4): two parallel requests from the same browser fired seconds apart do not legitimately drift fingerprints, so a mismatch within the grace window is treated as theft and triggers a full session revoke.
