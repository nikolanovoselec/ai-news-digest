...
5. Each invocation is bounded by a wall-clock budget rather than a fixed count of borderline judgments. When the budget is reached partway through a batch, the remaining borderline pairs in that batch stay distinct on this pass and are picked up on the next batch the sweep processes, so an operator-triggered sweep over a large corpus still converges without dropping borderline pairs silently. Per-invocation rerank counts are recorded in operator-visible logs so an operator can see how much rerank work each batch performed.
