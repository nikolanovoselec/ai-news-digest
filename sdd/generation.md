...
8. Multiple borderline pairs for one self are judged in a single batched language-model call rather than one call per pair, so a self with several borderline siblings does not multiply per-pair inference cost. A parse failure on the batched response is treated as "different events" for every pair in that batch, preserving the conservative default.
