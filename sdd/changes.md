...
- REQ-PIPE-016 changed: token and cost totals now include retried LLM attempts that consume tokens in the invalid-JSON path or hit the model tag-fanout guard, while article-ingestion and dedupe counters still advance only once per completed chunk.
