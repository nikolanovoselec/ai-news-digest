...
- REQ-PIPE-003 + REQ-PIPE-009 recalibrated against a 2026-05-07 prod audit: the same-story auto-merge bar is lowered so real same-news-cycle clusters that the prior 2026-05-06 calibration left below the bar (a PAN-OS zero-day cluster at 0.75-0.78, a Palo Alto Networks valuation-week cluster at 0.73-0.80) now collapse, and the rerank borderline band is widened on the lower edge to capture the lower tail of those same clusters; a per-batch rerank cap that was a leftover from the synchronous browser-loop era and was silently dropping borderline merges in large clusters is removed (the queue consumer's wall-clock budget already bounds the work per message).
