...
- REQ-PIPE-003 AC 11 refinement: the stricter same-publisher bar now applies only to articles whose URLs identify a real publisher; articles routed through an aggregator wrapper (currently news.google.com) no longer pay the same-publisher penalty, because the wrapper host carries no publisher identity and the penalty was suppressing legitimate cross-aggregator merges of the same story (Palo Alto / BTIG cluster on 2026-05-07 sat at cosine 0.86 across two GN redirect URLs and should have folded). Articles ingested in the same scrape tick that share an exact `published_at` second are now tie-broken by ULID so equal-time pairs can fold into each other instead of being silently kept apart by the strict-greater filter, and the per-item RSS publisher name surfaced on Google News items is preserved through to the source overlay instead of being clobbered by the feed-level aggregator label.
