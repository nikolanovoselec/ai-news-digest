...
- REQ-PIPE-008 AC 1 tightened: two studies, audits, or benchmarks on the same topic that cite different numbers, methodologies, or authors are treated as distinct stories and never merged by the cross-chunk dedup pass, so conflicting findings (e.g. two MCP-vulnerability studies citing 25% and 6.2%) remain visible as separate cards instead of risking a merge that hides one finding.
