...
3. The tapped chip then slides along a smooth path to its new sort position. When the user has just SELECTED the chip, the destination is the leftmost slot of the railing so the active filters cluster at the front. When the user has just UN-SELECTED a previously selected chip, the destination is the chip's natural sort position among the non-selected chips (sorted by article count descending, then alphabetically) — the chip slides rightward back into the count hierarchy. The slide's duration is shaped so the on-screen portion of the chip's journey takes roughly the same amount of time whether the chip travels a short visible hop or a long mostly-off-screen one — far chips get a longer total duration in exchange for a comfortably trackable visible window, instead of flashing past the eye in a blur. The chips between the chip's old and new positions slide in the opposite direction on a faster, fixed-duration curve to fill the gap quickly, so the gap closes promptly even when the tapped chip's full journey is still in flight.
...
6. The railing's horizontal scroll position is preserved across the cascade — the system does not auto-scroll on tap. On a viewport that scrolls the railing horizontally, the tapped chip slides toward its destination and may exit off either edge of the visible area; the user navigates the railing manually to see chips that have moved off-screen. The railing never jumps to a different scroll position as a side effect of the tap. As a convenience, after a SELECT cascade (and only when the chip lands at slot 0), the next time the user starts to scroll the surrounding page downward, the railing smoothly scrolls back to its leftmost position so the just-selected chip is revealed at the start as the user begins to read — this fires once per tap and is cancelled if the user manually swipes the railing during it. Unselect cascades do not arm this convenience scroll because the chip lands mid-railing rather than at slot 0.
