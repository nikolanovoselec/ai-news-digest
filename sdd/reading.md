...
3. The tapped chip then slides leftward to the start of the railing along a smooth path; the slide's duration scales with the distance the chip has to travel so the perceived velocity is uniform whether the chip moves a short hop or all the way across a long railing — a chip that has to travel further takes proportionally longer rather than racing past as a blur. Chips that previously sat to its left in the new order slide rightward to fill the slot it vacated on the same duration so the motion reads as one continuous cascade.
...
6. The railing's horizontal scroll position is preserved across the cascade — the system does not auto-scroll on tap. On a viewport that scrolls the railing horizontally, the tapped chip slides leftward and may exit off the left edge of the visible area; the user navigates the railing manually to see chips that have moved off-screen. The railing never jumps to a different scroll position as a side effect of the tap. As a convenience, the next time the user starts to scroll the surrounding page downward after a tap, the railing smoothly scrolls back to its leftmost position so the just-selected chip is revealed at the start as the user begins to read — this fires once per tap and is cancelled if the user manually swipes the railing during it.
