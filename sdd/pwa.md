...
3. Navigation is consolidated into the header on every viewport: brand on the left, and on the right a standalone theme-toggle icon immediately followed by an avatar-triggered user menu. The user menu contains a "Search & History" entry, Starred, Settings, and Log out — the theme toggle is no longer an item inside the menu. No separate sidebar or bottom tab bar, and no standalone header buttons beyond the brand, theme toggle, and avatar.
4. The digest is the app home; clicking the brand returns to it when signed in. When the user is already on the digest, clicking the brand scrolls the page to the top instead of triggering a self-navigation, so the wordmark also acts as a "back to top" affordance once the user has scrolled into the list.
