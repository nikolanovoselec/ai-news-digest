...
| LLM | Cloudflare AI Gateway + Workers AI | Single global default model route in production; AI Gateway Dynamic Routing (`dynamic/news_digest`) handles summarisation, discovery, and rerank; Workers AI supplies embeddings. Prompts are centralised in one module so all LLM-facing strings flow through one allowlist. |
