...
| **AI Gateway** | Cloudflare gateway used for the current Gemini-backed summarisation, discovery, and rerank LLM calls. The provider key stays in Gateway; the Worker uses a dedicated runtime Gateway token. |
| **Workers AI** | Cloudflare inference binding used for bge-base-en-v1.5 article embeddings and as the non-Gateway fallback path. |
| **ULID** | 26-character Crockford-base32 identifier used for article and scrape-run primary keys; sortable by creation time. |
| **First ingestion** | The scrape tick at which an article's canonical URL first entered the article pool. Subsequent re-discoveries on later ticks append the new source to the article's source list but never re-stamp this timestamp. The dashboard orders by first ingestion descending, so order reflects when each story was new to the pool — older stories that keep being re-broadcast by feeds do not displace genuinely fresher arrivals. |
