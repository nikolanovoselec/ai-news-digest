...
export async function dispatchDailyEmails(env: Env): Promise<void> {
  ...
  // Distinct tz probe: loop once per tz so each user's local clock is
  // computed from the same Intl.DateTimeFormat call, and the per-tz
  // query binds integer hour/minute bounds SQLite can index against.
  let tzRows: TzRow[];
  try {
...
/**
 * Run one per-tz dispatch pass: validate the timezone, compute the
 * 5-minute window, load eligible users, and send each their email.
 */
async function dispatchForTz(env: Env, tz: string, now: number): Promise<void> {
  // Defence-in-depth: even after the SQL filter, an unrecognised IANA
  // name (e.g. a deprecated zone the runtime no longer knows) would
  // throw inside localHourMinuteInTz and abort the whole tick.
  if (!isValidTz(tz)) {
  ...
  const users = await loadDispatchUsers(env.DB, tz, hour, bucketStart, bucketEnd, localDate);
  if (users === null) return; // error already logged
...
/**
 * Load users eligible for the current 5-minute dispatch window in `tz`.
 *
 * Returns the user rows on success, or `null` on a D1 error (already logged).
 */
...
/**
 * Fetch headlines + tally for one user, render their email, send it, and
 * stamp `last_emailed_local_date` on success.
 *
 * Never throws — all errors are logged and the function returns normally
 * so the outer loop continues with the next user.
 */
async function processOneUser(
  env: Env,
  user: DispatchUserRow,
  localDate: string,
  sinceMidnightUnix: number,
): Promise<void> {
  try {
    ...
    // Skip when there is nothing new to surface. Covers three shapes
    // that all collapse to "no signal": no tags, no matching articles
    // today, or a failed headlines read. last_emailed_local_date is left
    // untouched so the SQL gate accurately reflects the last send date.
    ...
    const { subject, text, html } = renderDigestReadyEmail({
      ...
      headlines,
      tagTally: tally,
      // Coerce "we don't know" sentinel back to 0 at the render boundary.
    ...
    if (!result.sent) {
      // Already logged inside sendEmail. Skip the date stamp so the
      // next cron tick retries within the same local day.
      return;
    }
    ...
    // Defensive: sendEmail is documented as non-throwing, and the UPDATE
    // is wrapped here so a D1 hiccup on one row doesn't abort the loop.
    log('error', 'email.send.failed', {
      user_id: user.id,
      status: null,
