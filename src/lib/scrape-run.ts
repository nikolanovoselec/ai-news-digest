 ...
 *
 * CF-021: `chunk_count` is intentionally NOT exposed on the seed.
 * The coordinator's duplicate-dispatch race guard relies on
 * `chunk_count > 0` meaning "fan-out happened" — every fresh row
 * MUST start at 0 and only the coordinator may UPDATE it after
 * fan-out. Allowing a seed value would silently break that invariant
 * with no test signal. */
...
export async function startRun(db: D1Database, seed: ScrapeRunSeed): Promise<void> {
  ...
  // CF-021 — `chunk_count` is always seeded at 0; the coordinator
  // is the sole writer that flips it positive after fan-out, which
  // is the load-bearing invariant for the duplicate-dispatch race
  // guard. Do NOT introduce a non-zero seed path.
  await db
    .prepare(
      `INSERT INTO scrape_runs (id, model_id, started_at, status, chunk_count)
       VALUES (?1, ?2, ?3, 'running', 0)`,
    )
    .bind(seed.id, seed.model_id, now)
    .run();
}
